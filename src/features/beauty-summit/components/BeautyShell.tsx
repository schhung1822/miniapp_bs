...
const BeautyShell: React.FC<BeautyShellProps> = ({ children, toast }) => {
  return (
    <div className="beauty-shell relative h-full overflow-hidden text-white">
      ...
      {children}
      {toast ? (
        <div className="beauty-toast pointer-events-none absolute left-1/2 bottom-28 z-50 -translate-x-1/2 rounded-full bg-emerald-500 px-4 py-2 text-center text-xs font-semibold text-white shadow-[0_14px_36px_rgba(16,185,129,0.35)]">
          {toast}
        </div>
      ) : null}
    </div>
